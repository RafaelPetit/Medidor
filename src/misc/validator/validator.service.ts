...
import { UploadMeasureDto } from "src/measurement/dto/measurement.dto";
...
export class validatorService {
    ...
    async validateMeasureData(uploadMeasureDto: UploadMeasureDto) {
        ...
        if (
          !this.isValidDate(uploadMeasureDto.measure_datetime)
        ) {
          throw new BadRequestException({
            error_code: 'INVALID_DATA',
      ...
      async existingMeasure(uploadMeasureDto: UploadMeasureDto): Promise<boolean> {
        return await this.measurementService.findByMonth(uploadMeasureDto.measure_datetime, uploadMeasureDto.measure_type);
      }
